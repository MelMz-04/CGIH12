...
void Animation() {
	...
	if (AnimDog) 
	{
		...
		// Trote diagonal: FLegs avanza, RLegs va opuesta
		if (!step)
		{
			FLegs += 2.0f;
			RLegs += 2.0f;
		...
		// Cabeza oscila en Z sincronizada con el paso
		head = 10.0f * sin(glm::radians(FLegs * 2.0f));

		// Cola oscila en Z más rápido y con más amplitud
		tail = 20.0f * sin(glm::radians(FLegs * 3.0f));
	}
	else // <<< CAMBIO: estado IDLE, todo regresa a cero
	{
		FLegs = 0.0f;
		RLegs = 0.0f;
		head = 0.0f;

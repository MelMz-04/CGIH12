...
int main()
{
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        // Set lights properties 2
        glUniform3f(glGetUniformLocation(lightingShader.Program, "light2.ambient"), 0.5f, 0.5f, 0.5f);
        glUniform3f(glGetUniformLocation(lightingShader.Program, "light2.diffuse"), 0.5f, 0.0f, 0.0f);
        ...
        model = glm::mat4(1.0f);
        model = glm::translate(model, lightPos + movelightPos2);
        model = glm::scale(model, glm::vec3(0.3f));
        glUniformMatrix4fv(glGetUniformLocation(lampshader.Program, "model"), 1, GL_FALSE, glm::value_ptr(model));
        glBindVertexArray(VAO);
        glDrawArrays(GL_TRIANGLES, 0, 36);
